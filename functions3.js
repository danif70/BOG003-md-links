//todo funciona bien pero no es asíncrono
const fs = require('fs');
...
//Aquí se recibe la ruta del usuario y se resuelve a absoluta con path.resolve, y se invoca la función isDirectory con esta ruta absoluta como parámetro
const mdLinks = () => {
...
//Función que utiliza el paquete markdownLinkExtractor para extraer links de los archivos .md
const linksOnMD = (arrayOfFiles) => {
  fs.readFile(arrayOfFiles, 'utf-8', (error, datos)=> {
    //console.log('linea 75 ',datos)
    if (error){console.log(error)}
    const links = markdownLinkExtractor(datos, false); //con el parámetro en true hay que revisar los condicionales ya que esto devuelve un array de objetos, y no un array de strings como devuelve false
    let newLinks = [];
    links.forEach((link) => {
